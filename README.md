...
(function() {
    ...
    const config = {
        ...
        size: {
            min: 2,
            max: 8,
            variance: 0.6 // Разброс размеров (0-1)
        },
        ...
        wind: {
            base: 0.4,
            variance: 0.3,
            changeFrequency: 0.001 // Частота изменения направления
        },
        ...
        colors: [
            ...
            'rgba(255, 250, 220, 0.7)',   // Теплый белый
            'rgba(230, 240, 255, 0.85)'   // Холодный белый
        ],
        ...
        glow: true,
        sparkle: true,
        trails: false, // Следы от снежинок
        
        // Формы снежинок
        shapes: ['circle', 'blur', 'star', 'crystal'],
        shapeDistribution: [0.5, 0.2, 0.15, 0.15], // Распределение форм
        ...
        // Взаимодействие
        interactive: true,
        followMouse: false, // Снежинки реагируют на мышь
        mouseRadius: 100    // Радиус влияния мыши
    };
    
    let snowflakes = [];
    let animationId = null;
    let isRunning = config.autoStart;
    let mouseX = 0;
    let mouseY = 0;
    let windDirection = 0;
    ...
    style.textContent = `
        ...
        .snowflake-beautiful.visible {
            opacity: var(--opacity);
        }
        
        /* Формы снежинок */
        .snowflake-beautiful.circle {
            border-radius: 50%;
            background: var(--color);
        }
        
        .snowflake-beautiful.blur {
            border-radius: 50%;
        ...
        /* Эффекты свечения */
        .snowflake-beautiful.glow {
            filter: drop-shadow(0 0 2px rgba(255, 255, 255, 0.5));
        }
        
        .snowflake-beautiful.sparkle {
            animation: sparkle 3s infinite alternate;
        }
        
        /* Эффект следа */
        .snowflake-beautiful.trail {
            position: absolute;
            opacity: 0.1;
            filter: blur(1px);
        }
        ...
        @keyframes sparkle {
            ...
            100% { opacity: 1; filter: drop-shadow(0 0 3px rgba(255, 255, 255, 0.8)); }
        }
        ...
        /* Эффект вьюги */
        .blizzard-mode .snowflake-beautiful {
            animation: windShift 2s infinite ease-in-out;
        }
        
        /* Тонкие линии на снежинках (для кристаллов) */
        .snowflake-beautiful.crystal:before {
            content: '';
            position: absolute;
            top: 0;
            left: 50%;
        ...
        .snowflake-beautiful.crystal:after {
            content: '';
            position: absolute;
            top: 50%;
            left: 0;
    ...
    function randomChoice(arr, distribution) {
    ...
    function createSnowflake() {
        ...
        // Выбор формы
        const shape = randomChoice(config.shapes, config.shapeDistribution);
        snowflake.className = `snowflake-beautiful ${shape}`;
        
        // Размер (нормальное распределение для реалистичности)
        let size;
        if (config.size.variance > 0) {
            size = gaussianRandom(
                (config.size.min + config.size.max) / 2,
                (config.size.max - config.size.min) * config.size.variance / 3
            );
            size = Math.max(config.size.min, Math.min(config.size.max, size));
        } else {
            size = random(config.size.min, config.size.max);
        }
        
        // Цвет
        const color = config.colors[randomInt(0, config.colors.length)];
        ...
        if (config.blur && shape !== 'blur') {
            snowflake.style.filter = 'blur(0.3px)';
        }
        
        // Вращение
        snowflake.style.transform = `rotate(${random(0, 360)}deg)`;
        
        document.body.appendChild(snowflake);
        
        // Плавное появление
        setTimeout(() => snowflake.classList.add('visible'), random(0, 500));
        ...
        snowflakes.push({
            ...
            // Движение
            speed: random(
                config.speed.base * (1 - config.speed.variance),
                config.speed.base * (1 + config.speed.variance)
            ),
            wind: random(
                config.wind.base * (1 - config.wind.variance),
                config.wind.base * (1 + config.wind.variance)
            ),
            
            // Анимация
            swing: random(0, Math.PI * 2),
            swingSpeed: random(0.01, 0.05),
            rotation: parseFloat(snowflake.style.transform.match(/rotate\(([-\d.]+)deg\)/)?.[1] || 0),
            rotationSpeed: random(-0.5, 0.5),
            
            // Дополнительные параметры
            shape: shape,
            drift: random(-0.2, 0.2), // Дополнительное боковое смещение
            wave: random(0, Math.PI * 2), // Для волнообразного движения
    ...
    function animateSnowflakes() {
        ...
        windDirection += random(-config.wind.changeFrequency, config.wind.changeFrequency);
        
        snowflakes.forEach((flake, index) => {
            // Основное движение вниз
            flake.y += flake.speed;
            
            // Боковое движение (ветер + индивидуальный дрейф)
            let windEffect = Math.sin(windDirection) * flake.wind;
            flake.x += windEffect + flake.drift;
            
            // Колебания (плавные волны)
            flake.swing += flake.swingSpeed;
            flake.x += Math.sin(flake.swing) * 0.3;
            
            // Вертикальные волны
            flake.wave += flake.waveSpeed;
            flake.y += Math.sin(flake.wave) * 0.1;
            
            // Вращение
            flake.rotation += flake.rotationSpeed;
            
            // Взаимодействие с мышью
            if (config.followMouse && config.interactive) {
                ...
                if (distance < config.mouseRadius) {
                    ...
                    flake.x -= Math.cos(angle) * force * 2;
                    flake.y -= Math.sin(angle) * force * 2;
                }
            }
            
            // Эффект следа
            if (config.trails) {
                flake.trailHistory.unshift({x: flake.x, y: flake.y});
                if (flake.trailHistory.length > flake.maxTrails) {
                    flake.trailHistory.pop();
                }
            }
            ...
            // Легкое мерцание прозрачности
            if (config.sparkle && Math.random() < 0.01) {
                flake.element.style.opacity = random(0.7, 1);
                setTimeout(() => {
    ...
    function resetSnowflake(flake) {
        ...
        flake.rotationSpeed = random(-0.5, 0.5);
    }
    ...
    function createControls() {
        // Кнопка переключения
        const toggleBtn = document.createElement('button');
        toggleBtn.className = 'snow-control';
        toggleBtn.innerHTML = '❄️';
        toggleBtn.title = 'Управление снегопадом';
        
        // Панель настроек
        const panel = document.createElement('div');
        panel.className = 'snow-panel';
        
        panel.innerHTML = `
            <h3>Настройки снегопада</h3>
            
            <div class="snow-slider-group">
                <label class="snow-slider-label">
                    Количество снежинок: <span class="snow-value" id="snowCountValue">${config.count}</span>
                </label>
                <input type="range" min="20" max="300" value="${config.count}" 
                       class="snow-slider" id="snowCountSlider">
            </div>
            
            <div class="snow-slider-group">
                <label class="snow-slider-label">
                    Скорость: <span class="snow-value" id="snowSpeedValue">${config.speed.base}</span>
                </label>
                <input type="range" min="0.5" max="3" step="0.1" value="${config.speed.base}" 
                       class="snow-slider" id="snowSpeedSlider">
            </div>
            
            <div class="snow-slider-group">
                <label class="snow-slider-label">
                    Размер: <span class="snow-value" id="snowSizeValue">${(config.size.min + config.size.max)/2}</span>
                </label>
                <input type="range" min="1" max="15" step="0.5" value="${(config.size.min + config.size.max)/2}" 
                       class="snow-slider" id="snowSizeSlider">
            </div>
            
            <div class="snow-checkbox">
                <input type="checkbox" id="snowGlow" ${config.glow ? 'checked' : ''}>
                <label class="snow-checkbox-label" for="snowGlow">Свечение</label>
            </div>
            
            <div class="snow-checkbox">
                <input type="checkbox" id="snowSparkle" ${config.sparkle ? 'checked' : ''}>
                <label class="snow-checkbox-label" for="snowSparkle">Мерцание</label>
            </div>
            
            <div class="snow-checkbox">
                <input type="checkbox" id="snowFollowMouse" ${config.followMouse ? 'checked' : ''}>
                <label class="snow-checkbox-label" for="snowFollowMouse">Реакция на мышь</label>
            </div>
            
            <div class="snow-stats">
                <span>Снежинок: <span id="snowCurrentCount">${snowflakes.length}</span></span>
                <span>FPS: <span id="snowFPS">60</span></span>
            </div>
        `;
        ...
        toggleBtn.addEventListener('dblclick', function() {
            isRunning = !isRunning;
            this.classList.toggle('paused', !isRunning);
            this.innerHTML = isRunning ? '❄️' : '⛄';
        ...
        // Слайдеры
        document.getElementById('snowCountSlider').addEventListener('input', function(e) {
            const newCount = parseInt(e.target.value);
            config.count = newCount;
            document.getElementById('snowCountValue').textContent = newCount;
            updateSnowflakeCount();
        });
        
        document.getElementById('snowSpeedSlider').addEventListener('input', function(e) {
            const speed = parseFloat(e.target.value);
            config.speed.base = speed;
            document.getElementById('snowSpeedValue').textContent = speed.toFixed(1);
            updateSnowflakeSpeed();
        });
        
        document.getElementById('snowSizeSlider').addEventListener('input', function(e) {
            const size = parseFloat(e.target.value);
            config.size.min = size * 0.7;
            config.size.max = size * 1.3;
            document.getElementById('snowSizeValue').textContent = size.toFixed(1);
            updateSnowflakeSize();
        });
        
        // Чекбоксы
        document.getElementById('snowGlow').addEventListener('change', function(e) {
            config.glow = e.target.checked;
            updateSnowflakeEffects();
        });
        
        document.getElementById('snowSparkle').addEventListener('change', function(e) {
            config.sparkle = e.target.checked;
            updateSnowflakeEffects();
        });
        
        document.getElementById('snowFollowMouse').addEventListener('change', function(e) {
            config.followMouse = e.target.checked;
        });
    }
    
    // Обновление количества снежинок
    function updateSnowflakeCount() {
        ...
        if (currentCount < config.count) {
            ...
            // Удаляем лишние
            const toRemove = snowflakes.slice(config.count);
            ...
            snowflakes = snowflakes.slice(0, config.count);
        }
    ...
    function updateSnowflakeSize() {
        snowflakes.forEach(flake => {
            const newSize = random(config.size.min, config.size.max);
    ...
    function updateSnowflakeEffects() {
        snowflakes.forEach(flake => {
            if (config.glow) {
                flake.element.classList.add('glow');
            } else {
                flake.element.classList.remove('glow');
            }
            
            if (config.sparkle) {
                flake.element.classList.add('sparkle');
            } else {
                flake.element.classList.remove('sparkle');
            }
        });
    }
    ...
    function updateStats() {
        document.getElementById('snowCurrentCount').textContent = snowflakes.length;
        
        frames++;
        const currentTime = performance.now();
        if (currentTime >= lastTime + 1000) {
            document.getElementById('snowFPS').textContent = frames;
            frames = 0;
    ...
    function init() {
        ...
        // Добавляем тонкий зимний градиент на фон
        document.body.style.background = 
            'linear-gradient(180deg, rgba(240, 248, 255, 0.02) 0%, transparent 100%)';
        ...
        document.addEventListener('visibilitychange', function() {
            if (document.hidden) {
                ...
                // Возвращаем при фокусе
                document.addEventListener('visibilitychange', function handler() {
                    if (!document.hidden && wasRunning) {
                        isRunning = true;
                        animateSnowflakes();
...
<style>
    /* Легкие эффекты для всего сайта */
    body {
        position: relative;
        overflow-x: hidden;
    }
    
    /* Эффект морозного стекла на некоторых элементах */
    .frost-effect {
        backdrop-filter: blur(2px);
        background: rgba(255, 255, 255, 0.03);
    }
    
    /* Анимация мерцания для текста */
    ...
    h1, h2, h3 {
        animation: textTwinkle 3s infinite ease-in-out;
    }
    
    /* Эффект падения для изображений при наведении */
    img:hover {
        transform: translateY(-2px);
        transition: transform 0.3s ease;
        filter: drop-shadow(0 4px 8px rgba(0, 0, 255, 0.1));
    }
    
    /* Снежные тени для кнопок */
    button:not(.snow-control):hover {
        box-shadow: 
